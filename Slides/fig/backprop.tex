...
\begin{tikzpicture}
	...
	\foreach \i in {1,...,2}
		\node[visible on=<5->] at (4, -\i*1.2 -1.2) {$\frac{\partial \theta_i}{\partial J}$};
	%frame4
	\foreach \i in {1,...,2}
		\foreach \j in {1,...,3}
			\draw[bp,visible on=<6>] (output\i) -- (hidden\j);
	\foreach \i in {1,...,3}
		\node[visible on=<6->] at (2, -\i*1.2 -.6) {$\frac{\partial \theta_i}{\partial J}$};
	%frame5
	\foreach \i in {1,...,3}
		\foreach \j in {1,...,4}
			\draw[bp,visible on=<7>] (hidden\i) -- (input\j);
	\foreach \i in {1,...,4}
		\node[visible on=<7->] at (0, -\i*1.2) {$\frac{\partial \theta_i}{\partial J}$};
\end{tikzpicture}